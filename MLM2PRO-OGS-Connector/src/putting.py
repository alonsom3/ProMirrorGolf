...
class Putting:
    ...
    def putting_stop_start(self):
        if self.putting_device is not None:
            if self.putting_device.is_running():
                ...
                self.putting_device.device_worker.club_selected(self.main_window.sim_connection.current_club)
    ...
    def __putting_settings_cancelled(self):
        if self.putting_device is not None and self.main_window.sim_connection.connected:
            self.putting_device.reload_putting_rois()
