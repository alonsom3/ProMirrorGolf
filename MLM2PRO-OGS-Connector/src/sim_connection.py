...
from src.settings import Simulator
from src.connect_base import ConnectBase
from src.connect_gspro import GSProConnect
from src.connect_ogs import OpenGolfSimConnect
from src.worker_gspro_start import WorkerGSProStart
from src.worker_sim_messages import WorkerSimMessages
from src.worker_sim import WorkerSim
from src.log_message import LogMessageSystems, LogMessageTypes
from src.worker_thread import WorkerThread
from PySide6.QtCore import QProcess


class SimConnection(QObject):

    # simulator_api = Simulator.GSPRO
    disconnected_from_sim = Signal()
    connected_to_sim = Signal()

    club_selected = Signal(object)
    sim_message = Signal(object)
    # ogs_message = Signal(object)

    def __init__(self, main_window, simulator_api: Simulator):
        super(SimConnection, self).__init__()
        ...
        self.worker = None
        self.thread = None
        self.sim_messages_thread = None
        self.sim_messages_worker = None
        ...
        if self.simulator_api == Simulator.OPENGOLFSIM:
            self.sim_connect = OpenGolfSimConnect()
        else:
            self.sim_connect = GSProConnect(
        ...
        self.__setup_sim_messages_thread()

    def __sim_disconnected(self):
        self.main_window.gspro_connect_button.setEnabled(True)
        self.main_window.log_message(LogMessageTypes.ALL, LogMessageSystems.GSPRO_CONNECT, f'Disconnected from {self.simulator_api}')
    ...
    def __setup_send_shot_thread(self):
        ...
        self.send_shot_worker = WorkerSim(self.sim_connect)
    ...
    def __setup_sim_messages_thread(self):
        self.sim_messages_thread = QThread()
        self.sim_messages_worker = WorkerSimMessages(self.sim_connect)
        self.sim_messages_worker.moveToThread(self.sim_messages_thread)
        self.sim_messages_worker.club_selected.connect(self.__club_selected)
        self.sim_messages_worker.error.connect(self.__sim_messages_error)
        self.sim_messages_worker.sim_message.connect(self.__sim_message)
        self.sim_messages_thread.started.connect(self.sim_messages_worker.run)
        self.sim_messages_thread.start()
    ...
    def __setup_connection_thread(self):
        self.thread = QThread()
        if self.simulator_api == Simulator.OPENGOLFSIM:
            self.worker = WorkerThread(self.sim_connect.init_socket, self.settings.ogs_ip_address, self.settings.ogs_port)
        else:
            self.worker = WorkerThread(self.sim_connect.init_socket, self.settings.ip_address, self.settings.port)
    ...
    def __club_selecion_error(self, error):
        ...
        QMessageBox.warning(self.main_window, f"{self.simulator_api} Receive Error", msg)
    ...
    def __send_shot_error(self, error):
        self.disconnect_sim()
        msg = f"Error while trying to send shot to {self.simulator_api}"
        self.__log_message(LogMessageTypes.LOGS, f'{msg}\nException: {format(error)}')
        QMessageBox.warning(self.main_window, f"{self.simulator_api} Send Error", msg)

    def __sim_messages_error(self, error):
        self.disconnect_sim()
        msg = f"Error while trying to check for new messages from GSPro.\nStart/restart API Connect from GSPro.\nPress 'Connect' to reconnect to GSPro."
        self.__log_message(LogMessageTypes.LOGS, f'{msg}\nException: {format(error)}')
        QMessageBox.warning(self.main_window, f"{self.simulator_api} Message Receive Error", msg)
        ...
        if not self.connected:
            if self.__find_gspro_api_app():
                if self.thread is None:
                    ...
                    if not self.sim_connect.connected():
                        self.worker.run()
                if self.sim_messages_thread is None:
                    self.__setup_sim_messages_thread()
                self.sim_messages_worker.start()
    ...
    def disconnect_sim(self):
        if self.connected:
            ...
            self.sim_messages_worker.stop()
            self.sim_connect.terminate_session()
            self.disconnected_from_sim.emit()

    def __sending_shot(self):
        self.__log_message(LogMessageTypes.ALL, 'Sending shot to {self.simulator_api}')

    def __in_progress(self):
        msg = 'Connecting...'
        self.__log_message(LogMessageTypes.ALL, f'Connecting to {self.simulator_api}...')
    ...
    def __connected(self):
        self.connected = True
        self.main_window.gspro_connect_button.setEnabled(True)
        self.main_window.log_message(LogMessageTypes.ALL, LogMessageSystems.GSPRO_CONNECT, f'Connected to {self.simulator_api}')
        ...
        self.connected_to_sim.emit()

    def __error(self, error):
        self.disconnect_sim()
        if self.simulator_api == Simulator.OPENGOLFSIM:
            msg = f"Error while trying to connect to {self.simulator_api}"
        else:
            msg = "Error while trying to connect to GSPro.\nMake sure GSPro API Connect is running.\nStart/restart API Connect from GSPro.\nPress 'Connect' to reconnect to GSPro."
        self.__log_message(LogMessageTypes.LOGS, f'{msg} Exception: {format(error)}')
        QMessageBox.warning(self.main_window, f"{self.simulator_api} Connect Error", msg)
    ...
    def __find_gspro_api_app(self):
        # assume opengolfsim is already running
        if self.simulator_api == Simulator.OPENGOLFSIM:
            running = True
            return running
    ...
    def __shutdown_threads(self):
        if self.sim_messages_thread is not None:
            self.sim_messages_worker.shutdown()
            self.sim_messages_thread.quit()
            self.sim_messages_thread.wait()
            self.sim_messages_thread = None
            self.sim_messages_worker = None
