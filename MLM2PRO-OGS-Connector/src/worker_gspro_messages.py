...
class WorkerGSProMessages(WorkerBase):
    ...
    def __init__(self, sim_connection: GSProConnect):
        super().__init__()
    ...
    def run(self):
        ...
        while not self._shutdown.is_set():
            ...
            self._pause.wait()
            if not self._shutdown.is_set() and self.sim_connection is not None and self.sim_connection.connected():
                try:
                    message = self.sim_connection.check_for_message()
                    if len(message) > 0:
